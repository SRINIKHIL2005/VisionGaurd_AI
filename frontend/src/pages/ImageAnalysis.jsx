          ...
          {imagePreview && (
            ...
            >
              ...
              <motion.button
                ...
                disabled={analyzing}
                className="w-full mt-4 bg-gradient-primary text-white py-4 rounded-xl font-semibold text-lg shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
              >
                {analyzing ? (
                  <>
        ...
        >
          ...
          {result && (
            <>
              ...
              {/* Visual Analysis Section - Show both Original and Annotated side by side */}
              {result.annotated_image ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {/* Annotated Image with AI Detections */}
                  <div className="bg-white rounded-2xl shadow-xl p-4 border-4 border-blue-500">
                    ...
                    />
                    <div className="mt-3 p-3 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-300">
                      <p className="text-xs font-bold text-gray-900 mb-2">🎯 Detection Legend:</p>
                      <div className="grid grid-cols-2 gap-2 text-xs text-gray-800">
                        <div>⭕ <span className="text-yellow-600 font-semibold">Yellow:</span> Known</div>
                        <div>⭕ <span className="text-purple-600 font-semibold">Magenta:</span> Unknown ⚠️</div>
                        <div>🔲 <span className="text-green-600 font-semibold">Green:</span> Normal</div>
                        <div>🔲 <span className="text-red-600 font-semibold">Red:</span> Weapon 🚨</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-yellow-50 border-2 border-yellow-400 rounded-xl p-6">
                  <h3 className="text-lg font-bold text-yellow-900 mb-2 flex items-center">
                    ⚠️ No Visual Analysis Available
                  </h3>
                  <p className="text-yellow-800 mb-4">
                    The backend did not return an annotated image. Check backend console for errors.
                  </p>
                  {/* Show original image as fallback */}
                  {imagePreview && (
                    <div className="mt-4">
                      <p className="text-sm text-yellow-700 mb-2">Original Image (no detections shown):</p>
                      <img
                        src={imagePreview}
                        alt="Original (no annotations)"
                        className="w-full rounded-xl shadow-md border-2 border-yellow-300"
                      />
                    </div>
                  )}
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Threat Classification</h3>
              ...
              {result.risk_assessment.threats && (
                <div className="space-y-3">
                  ...
                  {result.risk_assessment.threats.is_unknown_person && (
                    <div className="bg-gradient-warning text-gray-900 p-4 rounded-xl shadow-lg">
                      <div className="flex items-center space-x-3">
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl">
                    ...
                    <p className="text-sm text-gray-600 mt-2">Deepfake Status</p>
                    <p className="text-xs text-gray-500 mt-1">{result.deepfake.status.toUpperCase()}</p>
                    <p className="text-xs text-gray-500">{(result.deepfake.confidence * 100).toFixed(1)}% confidence</p>
                  </div>
                  
                  <div className="text-center p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl">
                    <p className="text-3xl font-bold text-purple-900">👤</p>
                    <p className="text-sm text-gray-600 mt-2">Identity</p>
                    <p className="text-xs font-semibold text-gray-700 mt-1">{result.face_recognition.identity}</p>
                    <p className="text-xs text-gray-500">{(result.face_recognition.confidence * 100).toFixed(1)}% match</p>
                    <p className="text-xs text-gray-500 mt-1">{result.face_recognition.num_faces} face(s)</p>
                  </div>
                  
                  <div className="text-center p-4 bg-gradient-to-br from-pink-50 to-pink-100 rounded-xl">
                    <p className="text-3xl font-bold text-pink-900">{result.objects.length}</p>
                    <p className="text-sm text-gray-600 mt-2">Objects Detected</p>
                    <p className="text-xs text-gray-500 mt-1">{result.suspicious_objects.length} suspicious</p>
